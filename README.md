...
- **Supersession classifier** (`classify_aged_equipment`): ST routinely keeps replaced equipment active, so old units are classified before flagging — zone pairing (old + recent unit share a zone token), "# of systems" custom-field arithmetic, tonnage pairing confirmed by sold capital estimates, and a booking-note counter-signal. Likely-replaced units render as a "Stale record likely" verify note (no red flag, 0 scorer points); ambiguous units get a softened "⚠ FLAG (verify)" (+10); only genuine remaining systems keep the full flag and points. Nothing is hidden — the tech always sees the record existed.
