...
def call_vision(key: str, image_path: Path, job_id: str) -> str:
    payload = {
...
def main(photo_manifest: str):
    ...
    for rec in data['manifest']:
        ...
        text=call_vision(key, sheet_path, job_id)
...
if __name__=='__main__':
    if len(sys.argv)<2:
        raise SystemExit('usage: analyze_photo_sheets_openrouter.py <photo_manifest.json>')
    main(sys.argv[1])
